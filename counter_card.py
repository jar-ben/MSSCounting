...
class Counter:
    def __init__(self, filename, useAutarky, useImu = True, useCard = True):
        self.variant = "base"
    ...
    def SS(self):
        ...
        if self.w5:
            ...
            clauses += self.W5(act)
        if self.useCard:
            clauses += self.card(max(self.maxVar, maxVar(clauses)))
    ...
    def card(self, current):
        current += 1
        limit = max(0, len(self.C) - 20)
        atMostRelaxed = 11
        atLeastRelaxed = 10
        print("card bound: {} -- {}".format(atLeastRelaxed, atMostRelaxed))
        activators = [i for i in range(1, len(self.C) + 1)]
        cnf = CardEnc.atmost(lits=activators, bound=atMostRelaxed, encoding=7, top_id = current).clauses
        current = maxVar(cnf) + 1
        cnf += CardEnc.atleast(lits=activators, bound=atLeastRelaxed, encoding=7, top_id = current).clauses
        return cnf
...
if __name__ == "__main__":
    ...
    parser.add_argument("--card", action='store_true', help = "Use cardinality encoding (define lower bound on the size of MSSes).")
    args = parser.parse_args()

    print(args.w2, args.w4, args.w5)

    counter = Counter(args.input_file, args.w2, args.imu, args.card)
